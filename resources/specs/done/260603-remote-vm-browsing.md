...
Detours users can browse and operate on files on their remote machines (development VMs, lab hosts, servers, and Intel Macs reached over SSH) from inside Detours with the same feel as a local pane. Connecting uses the SSH keys and configuration the user already has on their Mac, with no new passwords to manage and no SMB, NFS, AFP, Finder mount, or network-share setup for VM browsing. Once connected, the remote machine supports the file management features Detours has routed through provider-backed locations: file listings, Quick Look, Open With, folder sizes, git status markers, updates when files change, copy and move with progress, rename, new file/folder, Get Info, Copy Path, and safe deletion that can be undone. The feature is built for the case Marco uses every day: dev VMs and remote Macs on a fast local network, reached through the SSH agent and SSH config already on his Mac.

### Proposal

Add a Remote Hosts section to the sidebar where the user lists their SSH hosts. Selecting a host opens that host in a Detours pane with the same look and the same routed shortcuts as the local panes, including Quick Look, Open With, copy, move, safe delete, undo, new file/folder, Get Info, Copy Path, and git status markers. The remote feel is achieved by deploying a small helper program onto the host the first time the user connects, which Detours then talks to over the existing SSH connection, matching Redmargin's helper-over-SSH pattern.

### Behaviors

- Adding a remote host asks only for the SSH target (for example `wraith` or `marco@cognel-dev`). The target is also the label shown in the sidebar, breadcrumb, and operation messages. It does not ask for a display name, password, or key path; it uses the user's SSH agent and SSH configuration the same way the terminal does.
...
- Adding a host immediately connects that host and opens it in the current active pane. The sidebar entry is persisted at the same time so future connects can be started from Remote Hosts.
...
- Host trust follows the user's existing OpenSSH setup. If `ssh <target>` works in Terminal with the user's SSH config, known-hosts file, and agent, Detours must not show a separate fingerprint prompt before opening the host. If OpenSSH rejects the target, Detours surfaces the SSH failure instead of inventing a separate trust flow.
...
- The pane's breadcrumb shows a coloured pill with the host target as its leftmost element so the user always knows which host a pane is showing. File rows themselves look identical to local rows; there is no per-row remote badge.
...
- Copy, move, rename, New Folder, New File, Get Info, Copy Path, Quick Look, Open With, drag-out, Move to Trash, Undo restore, and Delete Immediately work for remote files. Operations that cross between local and remote or between remote hosts run as transfers in the operations queue with progress and cancel.
- Duplicate, Duplicate Structure, Archive, Extract Here, Reveal in Finder, and Share are local-only in the current app because those command paths still use local URL selections or macOS local-file services instead of provider-backed locations.
...
- Cmd-P quick navigation includes recently visited remote folders. Remote paths show the host target as a label so the user can tell them apart from local paths at a glance. Frecency entries are anchored to the host's identifier, so any future host-label changes can update existing entries without losing history. Entries from hosts that are not currently connected render dimmed.
...
- [x] **A1** The user can add a dev VM in under thirty seconds by typing a single SSH target such as `wraith`, with autocomplete suggestions drawn from the user's `~/.ssh/config`, and can connect successfully without entering a display name, password, or key path, using their existing SSH agent and SSH configuration.
...
- [x] **A6** Adding a remote host scans and records the host key before saving the host. Subsequent helper, transfer, and search SSH processes use strict host-key checking against the known-hosts file and fail closed on unknown or changed keys instead of prompting for a password or passphrase inside Detours.
...
- [x] **A8** Cmd-P quick navigation returns recently visited remote folders, shows the host target as a label, anchors history to the host identifier, and dims entries from hosts that are not currently connected.
...
- [x] **A20** A remote pane's breadcrumb shows a coloured pill with the host target as the leftmost element. No per-row visual indicator is added to file rows.
...
- Direct server-side remote-to-remote transfer between two different hosts. The app supports remote-to-remote copy/move by staging through the Mac in the provider transfer path.
...
The core change is introducing a `FileProvider` protocol that abstracts the filesystem behind every read and every write in Detours. Today `FileItem` carries a `Location` (either `local(URL)` or `remote(hostID, posixPath)`) and the data source, operations queue, rename controller, remote search, transfer materialization, and git status provider route through whichever `FileProvider` implements the current `Location`. Archive, extract, duplicate, duplicate-structure, Finder reveal, and Share remain local-URL command paths in the app UI.
...
Host trust and authentication are inherited from OpenSSH, matching Redmargin's helper-over-SSH pattern. Detours starts `/usr/bin/ssh` with the user's normal `~/.ssh/config`, known-hosts files, keys, agent, and ProxyJump behavior. Detours does not show an app-specific fingerprint prompt. Password, keyboard-interactive, and private-key passphrase prompts are not handled inside Detours; SSH must be set up so the target works non-interactively through the user's SSH agent/config.
...
| Drag-out to Finder or another app and the Open With round-trip both materialise files in `~/Library/Caches/Detours/remote/`. A hostname or path containing shell metacharacters used to construct a cache directory name can escape the cache root. | Cache directory names use short opaque host and session tokens, never raw host names or remote parent paths. The materialised leaf keeps the original filename after sanitisation so external editors show a recognisable document name. Per-session subdirectories are created with mode `0700`. Materialisation writes to a temp name and renames into place atomically. |
...
- [x] **T1** Create `src/Services/FileProvider/FileProvider.swift` defining the protocol with async methods: `list`, `stat`, `copy`, `move`, `delete`, `trash`, `restoreFromTrash`, `rename`, `archiveCreate`, `archiveExtract`, `createDirectory`, `watch`, `unwatch`, `gitStatus`, `folderSize`, `readSymlink`, `openForQuickLook`, `download`, `upload`, `version`, and `find`.
...
- [x] **T10** Route `src/Operations/RenameController.swift` and trash service through `FileProvider`. Archive create/extract server RPCs exist, but the app-level archive and extraction commands still require local URL selections.
...
- [x] **T23** Add `src/Remote/SSHHostTrust.swift` to keep remote-host connections on the user's normal OpenSSH trust path (`~/.ssh/config`, known hosts, ssh-agent) and reject interactive password/passphrase flows rather than prompting inside Detours.
...
- [x] **T28** Add `src/Sidebar/RemoteHostsSection.swift` and update `src/Sidebar/SidebarViewController.swift` and `src/Sidebar/SidebarItem.swift` with a new Remote Hosts section placed above the Network section. Each host row shows the SSH target as its label and a status dot.
...
- [x] **T30** Add `src/Sidebar/AddRemoteHostView.swift` (SwiftUI sheet) with a single SSH target field and autocomplete suggestions from `~/.ssh/config` top-level `Host` blocks (ignoring `Match` and conditional includes). Adding a host scans the host key before saving and records it for strict SSH checks.
- [x] **T31** Keep host-key trust fail-closed: the host record stores the known fingerprint, helper/transfer/search SSH invocations use strict known-host checking, and changed keys surface as connection failures instead of password/passphrase prompts.
...
- [x] **T34** Add the remote-pane breadcrumb host badge in `src/Panes/BreadcrumbView.swift` (or equivalent): coloured pill with the host target as the leftmost element of the breadcrumb. File rows are not modified.
...
- [x] **T41** Update `src/Navigation/` (Cmd-P quick nav, history, frecency) to store `Location` values anchored to `hostID`. Remote entries render with the host target label and are dimmed when the host is not connected.
- [x] **T42** Implement Open With round-trip in `src/FileList/FileOpenHelper.swift`: download remote file to `~/Library/Caches/Detours/remote/<short-host-token>/<short-session-token>/<filename>` (mode `0700`), record SHA256 and mtime, open via `NSWorkspace`, watch the local copy with `FSEventStream`, upload on save. On upload, recompute the remote SHA256 and mtime via daemon RPC; if either has changed, show a conflict dialog with Keep Mine, Keep Remote, Cancel and text naming whether the mismatch is content, timestamp, or both.
...
- [x] **T76** `RemoteIntegrationTests.testIntelMacPersistentSSHConnectionListsHome` - the same persistent `SSHConnection` + `SSHRemoteRPCClient` path used by the app connects to `ssh wraith`, starts the Intel macOS helper, performs protocol negotiation, and lists the remote home directory without a Detours fingerprint prompt.
...
- [x] **T88A** `SSHConnectionStateTests.testStateChangeNotificationPostsOnMainThread` - state-change notifications posted from background connection work are delivered on the main thread so AppKit observers do not trip main-actor executor checks.
- [x] **T89** `RemoteHostStoreTests.testPersistAcrossRelaunch` - hosts added to the store survive an `UserDefaults` reset round-trip.
- [x] **T89A** `RemoteHostStoreTests.testUpsertDeduplicatesBySSHTarget` and `testLoadDeduplicatesPersistedDuplicateTargets` - adding the same SSH target twice reuses one persisted host row and cleans duplicated persisted rows on next launch.
- [x] **T90** `RemoteHostTests.testCacheDirSanitisation` - a host label or SSH target containing shell metacharacters produces a cache directory name that never contains the raw characters.
- [x] **T91** `RemoteHostTests.testFrecencyAnchorsOnHostID` - host-label changes preserve the existing Cmd-P frecency entries and re-render them with the new label.
...
- [x] **T92A** `RemoteUISurfaceTests.testAddRemoteHostModelUsesSSHTargetOnly` and `testAddRemoteHostRequiresOnlyTarget` - Add Remote Host accepts a single SSH target, stores that target as the sidebar label, and does not require a separate display name.
