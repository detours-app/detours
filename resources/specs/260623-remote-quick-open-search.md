...
- **Server-side traversal.** Implement the name search in the helper (new `Server/FindOperations.swift`, invoked from `Server/RPCHandler.swift`). Traverse priority roots (`$HOME` read from the server environment, then `/opt`) first, each with single-filesystem semantics, then the rest of the root filesystem, de-duplicating paths already covered. Prune `/proc`, `/sys`, `/dev` and common noise dirs (`.git`, `node_modules`); do not follow symlinks; swallow permission-denied errors. Stop at the result cap (500 matches) or an internal time budget (5 seconds), whichever comes first, so a single `find` can never hold the sequential RPC connection open indefinitely. Emit matches in batches as they are found.
...
- **"Cancel in-flight" on a sequential RPC connection.** The helper processes one request at a time per connection, so true mid-flight preemption is not available. The client achieves responsive cancel by debouncing and only honoring the latest request's results; the helper bounds each `find` with a 500-match result cap and a 5-second internal time budget so the connection is freed promptly and never blocked by a slow whole-host walk. This is what makes A4 hold.
...
| Risk | Mitigation |
| ---- | ---------- |
| A slow whole-host `find` blocks other RPCs on the shared connection | Helper enforces a 500-match result cap and a 5-second internal time budget per `find`; it returns promptly and streams partial results |
