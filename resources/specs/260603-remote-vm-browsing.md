...
- [x] **T30** Add `src/Sidebar/AddRemoteHostView.swift` (SwiftUI sheet) with a single SSH target field, autocomplete suggestions from `~/.ssh/config` top-level `Host` blocks (ignoring `Match` and conditional includes), a Test Connection button, and the first-connect host-key fingerprint confirmation step.
